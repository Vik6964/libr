...
namespace JiraAddin
{
    public class JiraClient : IJiraClient
    {
        ...
        public IEnumerable<Issue> GetIssue(String projectKey, String issueKey)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (!String.IsNullOrEmpty(issueKey))
                        jql += String.Format(" AND issueKey={0}", WebUtility.HtmlEncode(issueKey));
        ...
        public IEnumerable<Issue> GetSubTasksOfIssue(String projectKey, String issueKey, String issueType)
        {
            try
            {
                var result = new List<Issue>(4);
                while (true)
                {
                    ...
                    if (!String.IsNullOrEmpty(issueKey))
                        jql += String.Format(" AND parent={0}", WebUtility.HtmlEncode(issueKey));

                    if (!String.IsNullOrEmpty(issueType))
                        jql += String.Format(" AND Type={0}", WebUtility.HtmlEncode(issueType));
        ...
        public Issue CreateSubTask(String projectKey, String parentKey, String issueType, String summary, String description)
        {
            try
            {
                var request = CreateRequest(Method.POST, "issue");
        ...
        public Issue UpdateIssue(Issue issue)
        {
            try
            {
             ...
             //       update.Add("labels", new[] { new { set = issue.fields.labels } });
               ...
               // if (issue.fields.comments != null)
               //     update.Add("comments", new[] { new { set = issue.fields.comments } });
